...
class GapDetector:
    ...
    def detect_gaps(
        ...
        LEFT JOIN lineage_edges e_in ON o.object_id = e_in.target_id AND e_in.edge_type = 'dependency'
        LEFT JOIN lineage_edges e_out ON o.object_id = e_out.source_id AND e_out.edge_type = 'dependency'
        WHERE
            (
                m.object_id IS NULL  -- Never parsed
                OR (e_in.source_id IS NULL AND e_out.target_id IS NULL)  -- No dependencies found in edge table (v0.10.0 Phase 4.4)
            )
        """
    ...
    def get_gap_statistics(self) -> Dict[str, Any]:
        ...
        WITH object_deps AS (
            SELECT DISTINCT
                o.object_id,
                m.object_id IS NOT NULL as has_metadata,
                EXISTS(SELECT 1 FROM lineage_edges e
                       WHERE (e.source_id = o.object_id OR e.target_id = o.object_id)
                       AND e.edge_type = 'dependency') as has_deps
            FROM objects o
            LEFT JOIN lineage_metadata m ON o.object_id = m.object_id
        )
        SELECT
            COUNT(*) as total_objects,
            SUM(CASE WHEN has_metadata THEN 1 ELSE 0 END) as parsed_objects,
            SUM(CASE WHEN NOT has_metadata THEN 1 ELSE 0 END) as unparsed_objects,
            SUM(CASE WHEN has_metadata AND NOT has_deps THEN 1 ELSE 0 END) as empty_dependencies
        FROM object_deps
        """
        ...
        SELECT
            object_type,
            COUNT(*) as total,
            SUM(CASE WHEN has_metadata THEN 1 ELSE 0 END) as parsed,
            SUM(CASE WHEN NOT has_metadata THEN 1 ELSE 0 END) as unparsed,
            SUM(CASE WHEN has_metadata AND NOT has_deps THEN 1 ELSE 0 END) as empty
        FROM object_deps
