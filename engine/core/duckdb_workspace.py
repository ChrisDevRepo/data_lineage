...
from .validation import validate_parquet_schema, PARQUET_SCHEMAS
...
class DuckDBWorkspace:
    ...
    SCHEMA_LINEAGE_EDGES = """
        ...
        CREATE INDEX IF NOT EXISTS idx_edges_source ON lineage_edges(source_id);
        CREATE INDEX IF NOT EXISTS idx_edges_target ON lineage_edges(target_id);
        CREATE INDEX IF NOT EXISTS idx_edges_type ON lineage_edges(edge_type);
    """
    ...
    def _initialize_schema(self):
        ...
        try:
            ...
            if 'found_tables' not in column_names:
                ...
                """)
                logger.info("✓ Schema update complete: found_tables column added")
        ...
        try:
            ...
            if 'parse_failure_reason' not in column_names:
                logger.info("Schema update: adding parse_failure_reason column...")
                self.connection.execute("""
                    ALTER TABLE lineage_metadata
                    ADD COLUMN parse_failure_reason VARCHAR
                """)
                logger.info("✓ Schema update complete: parse_failure_reason column added")

            # Add expected_count column
            if 'expected_count' not in column_names:
                logger.info("Schema update: adding expected_count column...")
                self.connection.execute("""
                    ALTER TABLE lineage_metadata
                    ADD COLUMN expected_count INTEGER
                """)
                logger.info("✓ Schema update complete: expected_count column added")

            # Add found_count column
            if 'found_count' not in column_names:
                logger.info("Schema update: adding found_count column...")
                self.connection.execute("""
                    ALTER TABLE lineage_metadata
                    ADD COLUMN found_count INTEGER
                """)
                logger.info("✓ Schema update complete: found_count column added")
        except Exception as e:
        ...
        try:
            ...
            if 'dependencies' in table_names:
                ...
                if 'referenced_id' not in column_names:
                    logger.info("Schema update: adding referenced_id column to dependencies table...")
                    self.connection.execute("""
                        ALTER TABLE dependencies
                        ADD COLUMN referenced_id BIGINT
                    """)
                    logger.info("✓ Schema update complete: referenced_id column added")
        except Exception as e:
    ...
    def load_parquet_from_mappings(
        ...
        # DEBUG: Verify tables exist and have data before returning
        logger.info(f"[DEBUG] load_parquet_from_mappings: Verifying table counts before return")
        for table_name in required_tables:
            try:
                verify_result = self.connection.execute(f"SELECT COUNT(*) FROM {table_name}").fetchone()
                verify_count = verify_result[0] if verify_result else 0
                logger.info(f"[DEBUG]   {table_name}: {verify_count} rows (reported: {row_counts.get(table_name, 'N/A')})")

                # CRITICAL: If verification count doesn't match reported count, this is the bug!
                if verify_count != row_counts.get(table_name, 0):
                    logger.error(f"[DEBUG] ❌ MISMATCH DETECTED! Table {table_name} has {verify_count} rows but reported {row_counts.get(table_name, 0)}")
            except Exception as e:
                logger.error(f"[DEBUG] Failed to verify {table_name}: {e}")
    ...
    def _detect_table_type(self, file_path: Path) -> str | None:
        ...
        Validates against INTERFACE 2 schema contract (docs/INTERFACE_CONTRACTS.md).
        ...
        try:
            ...
            # Match against known table schemas (validates column presence)
            if PARQUET_SCHEMAS['objects'].issubset(columns):
                return 'objects'
            elif PARQUET_SCHEMAS['dependencies'].issubset(columns):
                return 'dependencies'
            elif PARQUET_SCHEMAS['definitions'].issubset(columns):
                return 'definitions'
    ...
    def _load_parquet_file(
        ...
        logger.info(f"[DEBUG] _load_parquet_file: Loading {table_name} from {file_path}")

        # Drop table if replace requested
        if replace:
            self.connection.execute(f"DROP TABLE IF EXISTS {table_name}")
            logger.info(f"[DEBUG] Dropped existing table: {table_name}")
        ...
        logger.info(f"[DEBUG] CREATE OR REPLACE TABLE executed for {table_name}")

        # Force checkpoint to persist data immediately (DuckDB WAL sync issue fix)
        # Reference: https://github.com/duckdb/duckdb/issues/9721
        try:
            self.connection.execute("CHECKPOINT")
            logger.info(f"[DEBUG] CHECKPOINT executed after creating {table_name}")
        except Exception as e:
            logger.warning(f"[DEBUG] CHECKPOINT failed (may not be critical): {e}")
        ...
        logger.info(f"[DEBUG] Row count for {table_name}: {row_count}")
    ...
    def get_objects_to_parse(
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"[DEBUG] get_objects_to_parse called with full_refresh={full_refresh}")
        ...
        if full_refresh:
            ...
            """
            logger.info(f"[DEBUG] Executing full_refresh query for all objects")
        else:
        ...
        logger.info(f"[DEBUG] Executing query: {query[:100]}...")
        ...
        objects_list = [dict(zip(columns, row)) for row in results]
        logger.info(f"[DEBUG] Returning {len(objects_list)} objects for parsing")
        return objects_list
    ...
    def update_metadata(
        ...
        """
        Update lineage_metadata and edge table for an object using UNION merge strategy.

        v0.10.0 Phase 4.4: JSON columns removed - uses edge table exclusively.
        ...
        Args:
            ...
            inputs: List of input object_ids (written to edge table)
            outputs: List of output object_ids (written to edge table)
        ...
        # Read existing metadata if it exists (edge table only)
        existing = self.connection.execute("""
            SELECT primary_source, parse_success
            FROM lineage_metadata
            WHERE object_id = ?
        """, [object_id]).fetchall()

        # Get existing dependencies from edge table for UNION merge
        existing_inputs_result = self.connection.execute("""
            SELECT source_id FROM lineage_edges
            WHERE target_id = ? AND edge_type = 'dependency'
        """, [object_id]).fetchall()
        old_inputs = [row[0] for row in existing_inputs_result]

        existing_outputs_result = self.connection.execute("""
            SELECT target_id FROM lineage_edges
            WHERE source_id = ? AND edge_type = 'dependency'
        """, [object_id]).fetchall()
        old_outputs = [row[0] for row in existing_outputs_result]

        if existing and len(existing) > 0:
            # Existing record found - determine merge strategy
            old_source, old_parse_success = existing[0]
        ...
        # Write metadata (no JSON columns)
        query = """
        ...
        # Write to edge table (v0.10.0 Phase 4.4 - single source of truth)
        # First, delete existing edges for this object
        self.connection.execute("""
            DELETE FROM lineage_edges
            WHERE source_id = ? OR target_id = ?
        """, [object_id, object_id])

        # Insert new edges from inputs (dependency: input -> current object)
        for input_id in merged_inputs:
            self.connection.execute("""
                INSERT OR IGNORE INTO lineage_edges (source_id, target_id, edge_type)
                VALUES (?, ?, 'dependency')
            """, [input_id, object_id])

        # Insert new edges from outputs (dependency: current object -> output)
        for output_id in merged_outputs:
            self.connection.execute("""
                INSERT OR IGNORE INTO lineage_edges (source_id, target_id, edge_type)
                VALUES (?, ?, 'dependency')
            """, [object_id, output_id])
