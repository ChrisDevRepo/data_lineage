CREATE PROC [STAGING_CADENCE].[spLoadMonthlyTaskTaskCountry] AS
...
DECLARE @servername VARCHAR(100) = CAST( SERVERPROPERTY( 'ServerName' ) AS VARCHAR(100) )
  ,@ProcShortName NVARCHAR(128) = 'spLoadMonthlyTaskTaskCountry'
...
-- EV Allocation 
-- Calculate aggregate totals WHERE [Task Country Name] NOT IN ('Global', 'No Country')
...
insert 	INTO [STAGING_CADENCE].[AggregatedCountryTotals]
    SELECT 
		...
		, SUM(COALESCE(FTS.[SUM Task Period Approved Total Units], 0)) --* FTS.[Calculated Unit Cost]) 
    ...
    WHERE FTS.[Task Country Name] NOT IN ('Global', 'No Country')
    GROUP BY 
		  FTS.[Period Year]
		, FTS.[Period Month]
		, FTS.[Project Name]	
...
-- Create table with reacords aligned with aggregate total colunns  WHERE [Task Country Name] NOT IN ('Global', 'No Country')
IF OBJECT_ID(N'STAGING_CADENCE.MonthlyTaskTaskCountry') IS NOT NULL
    TRUNCATE TABLE STAGING_CADENCE.MonthlyTaskTaskCountry
    

insert 	INTO STAGING_CADENCE.MonthlyTaskTaskCountry
    SELECT 
    	  CT.[Period Year]
		, CT.[Period Month]
		, CT.[Project Name]	
		...
		, CASE WHEN	  CT.[RecordUpdateType] in ('NC', 'NCF', 'NCD', 'NCB')
		  ...
		  END AS [SUM Function Period TimeSheet Actual Total Hours]
		...
		, FTS.[Calc Total SUM Task Period Approved Total Units]AS [Total SUM Task Period Approved Total Units]
...
, SUM((COALESCE(CT.[SUM Task Period Approved Total Units], 0) --* CT.[Calculated Unit Cost])
	 ...
	 - (case when CT.[Task Name] like '%Project Management%' 
        ...
        WHERE CT.[Task Country Name] NOT IN ('Global', 'No Country')
        GROUP BY 
					  CT.[Period Year]
					, CT.[Period Month]
					, CT.[Project Name]
...
SELECT @MSG  = 'Completed load of [STAGING_CADENCE].[MonthlyTaskTaskCountry] in ' + CAST(DATEDIFF(second, @ProcessingTime, GETDATE()) AS VARCHAR(10)) + ' Seconds' ;
