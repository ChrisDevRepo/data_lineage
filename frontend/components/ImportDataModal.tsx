...
type ImportDataModalProps = {
    ...
    dbEnabledFromApp?: boolean; // Optional: database status from App mount (v0.10.1 - eliminates button delay)
};
...
type UploadMode = 'files' | 'database' | 'json';
...
export const ImportDataModal = ({ isOpen, onClose, onImport, currentData, defaultSampleData, addNotification, dbEnabledFromApp }: ImportDataModalProps) => {
    // JSON import state
    ...
    // Database refresh state (initialized from App-level fetch for instant availability)
    const [dbEnabled, setDbEnabled] = useState(dbEnabledFromApp || false);
    const [dbConnectionStatus, setDbConnectionStatus] = useState<'idle' | 'checking' | 'success' | 'error' | 'disabled'>('disabled');
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // When modal closes, check if we completed an import and need to refresh
            const hadSuccessfulImport = jobStatus?.status === 'completed';
            ...
            // If we had a successful import, force page reload to show new data
    ...
    const fetchMetadata = async () => {
        ...
        // Check database configuration and test connection
        // Always fetch to get server/database info and verify connectivity
        const checkDatabaseConnection = async () => {
            // If we already know DB is disabled from App prop, skip the check
            if (dbEnabledFromApp === false) {
                setDbEnabled(false);
            ...
            // Show checking status while we verify
            if (dbEnabledFromApp === true) {
                setDbEnabled(true);
                setDbConnectionStatus('checking');
                setDbConnectionMessage('Checking database connection...');
            }
            ...
            try {
                ...
                const dbResponse = await fetch(`${API_BASE_URL}/api/database/test-connection`, {
                ...
                if (dbResult.enabled) {
                    ...
                    // Build connection info message with server/database
                    let infoMessage = 'Database import available';
                    if (dbResult.server || dbResult.database) {
                        const parts = [];
                        if (dbResult.server) parts.push(`Server: ${dbResult.server}`);
                        if (dbResult.database) parts.push(`Database: ${dbResult.database}`);
                        infoMessage = `${infoMessage} (${parts.join(' | ')})`;
                    }

                    // Check if server is actually reachable
                    if (dbResult.success) {
                        setDbConnectionStatus('success');
                        setDbConnectionMessage(infoMessage + ' - Connected');
                    } else {
                        setDbConnectionStatus('idle');
                        setDbConnectionMessage(infoMessage);
                    }
                } else {
                    setDbEnabled(false);
                    setDbConnectionStatus('disabled');
                ...
                // If we knew it was enabled from props, show as idle (configured but couldn't verify)
                if (dbEnabledFromApp === true) {
                    setDbEnabled(true);
                    setDbConnectionStatus('idle');
                    setDbConnectionMessage('Database import available (connection check timed out)');
                } else {
                    setDbEnabled(false);
                    setDbConnectionStatus('disabled');
                    setDbConnectionMessage('Could not verify database configuration');
                }
            }
        };
    ...
    const handleApply = async () => {
        ...
        if (errors.length === 0) {
            ...
            onImport(data);
            // Close modal and refresh page after successful JSON import
            setTimeout(() => {
                onClose();
    ...
    const handleLoadSampleData = async () => {
        ...
        if (errors.length === 0) {
            ...
            // Close modal and refresh page after loading sample data
            setTimeout(() => {
                onClose();
                // Force page refresh to ensure clean state
                window.location.href = window.location.href.split('?')[0] + '?_reload=' + Date.now();
    ...
    const handleDatabaseRefresh = async () => {
        ...
        try {
            // First, test the database connection (with timeout)
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 60000); // 1 minute timeout

            const testResponse = await fetch(`${API_BASE_URL}/api/database/test-connection`, {
                credentials: 'same-origin',
                signal: controller.signal
            });
            clearTimeout(timeoutId);
            ...
            if (!testResult.enabled) {
                throw new Error('Database connection not enabled (set DB_ENABLED=true in .env)');
            }

            // Build connection info string for display
            let connectionInfo = '';
            if (testResult.server || testResult.database) {
                const parts = [];
                if (testResult.server) parts.push(`Server: ${testResult.server}`);
                if (testResult.database) parts.push(`Database: ${testResult.database}`);
                connectionInfo = ` (${parts.join(' | ')})`;
            }

            // Update job status with connection details
            setJobStatus({
                job_id: '',
                status: 'processing',
                progress: 2,
                message: `Checking database connection${connectionInfo}...`
            });

            if (!testResult.success) {
                throw new Error(testResult.message || 'Database not reachable');
            }

            // Update connection status to success
            setDbConnectionStatus('success');
            setDbConnectionMessage(testResult.message || 'Database connection successful');
            ...
            setJobStatus({
                ...
                status: 'processing',
                progress: 5,
                message: `Database connected${connectionInfo}, starting refresh...`
            });
            ...
            const { job_id, message } = refreshData;

            // Parse database import summary from response message
            // Example: "Database refresh started (full refresh mode). Imported: 13 Tables, 3 Views, 3 Stored Procedures, 1 Scalar Functions, 1 Table Functions, 1 Inline Functions (22 total)."
            if (message && message.includes('Imported:')) {
                const match = message.match(/Imported:\s*(.+?)\s*\(\d+\s*total\)/);
                if (match) {
                    const importedText = match[1];
                    const tableMatch = importedText.match(/(\d+)\s+Tables?/i);
                    const viewMatch = importedText.match(/(\d+)\s+Views?/i);
                    const spMatch = importedText.match(/(\d+)\s+Stored Procedures?/i);
                    const fnMatch = importedText.match(/(\d+)\s+(?:Scalar|Table|Inline)\s+Functions?/i);

                    // Create a summary object for database imports
                    const tableCount = tableMatch ? parseInt(tableMatch[1]) : 0;
                    const viewCount = viewMatch ? parseInt(viewMatch[1]) : 0;
                    const spCount = spMatch ? parseInt(spMatch[1]) : 0;
                    const fnCount = fnMatch ? parseInt(fnMatch[1]) : 0;
                    const totalObjects = tableCount + viewCount + spCount + fnCount;

                    const dbImportSummary = {
                        by_object_type: {
                            'Table': { total: tableCount },
                            'View': { total: viewCount },
                            'Stored Procedure': { total: spCount },
                            'Function': { total: fnCount }
                        },
                        coverage: totalObjects > 0 ? 100 : 0  // Database imports have 100% coverage
                    };
                    setParseSummary(dbImportSummary);
                    setShowSummary(true);
                }
            }
            ...
            setJobStatus({
                ...
                status: 'error',
                progress: 0,
                message: `Database refresh failed: ${errorMessage}`,
                errors: [errorMessage]
            });
            setIsProcessing(false);
        }
        ...
        const poll = async () => {
            try {
                ...
                if (status.status === 'completed') {
                    ...
                    // Reload fresh data from the workspace (critical for database import)
                    try {
                        const freshDataResponse = await fetch(`${API_BASE_URL}/api/latest-data?_t=${Date.now()}`, { credentials: 'same-origin' });
                        if (freshDataResponse.ok) {
                            const freshData = await freshDataResponse.json();
                            const freshLineageData: DataNode[] = freshData.data || freshData;
                            onImport(freshLineageData);
                        }
                    } catch (err) {
                        console.warn('Failed to reload fresh data after import:', err);
                        // Continue anyway - already imported data above
                    }
                    ...
                    // Don't auto-close - let user manually close to view results
                    // This matches the behavior of the Parquet upload tab
    ...
    const handleClearData = async () => {
        ...
        try {
            ...
            addNotification(`Failed to clear data: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
        } finally {
            setIsClearing(false);
        }
    };
            ...
            <div className="fixed inset-0 z-[51] flex items-center justify-center p-4 pointer-events-none">
                <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col text-gray-800 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                    ...
                    <header className="px-6 py-5 border-b border-gray-200">
                        <h2 className="text-xl font-semibold text-gray-900 mb-4">Import Metadata</h2>
                        <div className="flex gap-0 border-b border-gray-200">
                            {/* Database Tab - always enabled, shows warning if not configured */}
                            <button
                                onClick={() => {
                                    setUploadMode('database');
                                    // Clear errors/status when switching tabs (don't clear if completed with summary)
                    ...
                    {uploadMode === 'json' ? (
                                ...
                                {jobStatus && jobStatus.warnings && jobStatus.warnings.length > 0 && (
                                    <div className="border border-yellow-200 rounded-lg p-4 bg-yellow-50">
                                        ...
                                        <p className="text-xs text-yellow-700 mt-2">
                                            See Developer Panel (Help → Developer Panel) for detailed logs.
                                        </p>
                                    </div>
                                )}
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <span className="font-bold text-green-900">
                                                        ...
                                                        <span className="text-xs text-green-700 ml-2">
                                                            ({parseSummary.by_object_type['Table']?.total || 0} tables, {parseSummary.by_object_type['View']?.total || 0} views, {parseSummary.by_object_type['Stored Procedure']?.total || 0} SPs, {parseSummary.by_object_type['Function']?.total || 0} functions)
                                                        </span>
                                                    </span>
                                                </div>
                                                <div className="flex items-center justify-between">
                        ...
                        <main className="flex-grow p-4 overflow-y-auto">
                            <div className="max-w-2xl mx-auto space-y-4">
                                {/* Database not configured warning - shown prominently when DB_ENABLED=false */}
                                {!dbEnabled && (
                                    <div className="bg-amber-50 border-2 border-amber-300 rounded-lg p-4">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-semibold text-amber-900">Database Connection Not Configured</p>
                                                <p className="text-sm text-amber-800 mt-1">
                                                    To enable database import, add the following to your <code className="bg-amber-100 px-1 rounded">.env</code> file:
                                                </p>
                                                <pre className="mt-2 bg-amber-100 p-2 rounded text-xs font-mono text-amber-900 overflow-x-auto">
                                                    {`DB_ENABLED=true
DB_CONNECTION_STRING="DRIVER={ODBC Driver 18 for SQL Server};SERVER=your-server;DATABASE=your-db;UID=user;PWD=pass"`}
                                                </pre>
                                ...
                                {/* Database connection status banner - only show when DB is enabled */}
                                {dbEnabled && dbConnectionStatus === 'success' ? (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                                        <div className="flex items-center gap-2">
                                ...
                                ) : dbEnabled && dbConnectionStatus === 'error' ? (
                                ...
                                ) : dbEnabled && dbConnectionStatus === 'checking' ? (
                                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <p className="text-sm text-blue-700">{dbConnectionMessage || 'Database import available'}</p>
                                        </div>
                                    </div>
                                ) : dbEnabled && dbConnectionStatus === 'idle' ? (
                                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                        <p className="text-sm text-blue-700">{dbConnectionMessage || 'Database import available'}</p>
                                    </div>
                                ) : null}
                                ...
                                {/* Database refresh instructions - only show when DB is enabled */}
                                {dbEnabled && (
                                    <>
                                        <div className="border border-gray-300 rounded-lg p-3">
                                ...
                                {/* Refresh button - always visible, disabled when not configured or connection issues */}
                                <Button
                                    onClick={handleDatabaseRefresh}
                                    disabled={!dbEnabled || isProcessing || dbConnectionStatus === 'checking' || dbConnectionStatus === 'error' || dbConnectionStatus === 'disabled'}
                                    variant="primary"
                                    fullWidth
                                    className="py-3"
                                >
                                    {isProcessing ? 'Refreshing...' :
                                        !dbEnabled || dbConnectionStatus === 'disabled' ? 'Database Not Configured' :
                                            dbConnectionStatus === 'checking' ? 'Checking connection...' :
                                                dbConnectionStatus === 'error' ? 'Connection Failed' :
