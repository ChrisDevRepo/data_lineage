...
export const Legend = React.memo(
  ({
    ...
    const schemasToShow = isSchemasExpanded
      ? filteredSchemas
              ...
              <div className="grid grid-cols-1 gap-y-1.5">
                {schemasToShow.map((s) => (
                  <div key={s} className="flex items-center gap-2" title={s}>
                    <span
                      style={{ backgroundColor: schemaColorMap.get(s) }}
                      className="w-3 h-3 inline-block rounded-sm ring-1 ring-black/20 flex-shrink-0"
                    ></span>
