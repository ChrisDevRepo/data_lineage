...
export function DeveloperPanel({ isOpen, onClose, dialect }: DeveloperPanelProps) {
  const [activeTab, setActiveTab] = useState<'logs' | 'tsql-rules' | 'default-rules'>('logs');
  ...
  // Fetch rules
  const fetchRules = async (targetDialect: string) => {
    try {
      setLoading(true);
      setError(null);
      // Reset selection when switching dialects
      setSelectedRule(null);
      setRuleContent('');

      const response = await fetch(`${API_BASE_URL}/api/rules/${targetDialect}`, { credentials: 'same-origin' });
  ...
  const fetchRuleContent = async (filename: string) => {
    try {
      setLoading(true);
      setError(null);
      const targetDialect = activeTab === 'default-rules' ? 'defaults' : dialect;
      const response = await fetch(`${API_BASE_URL}/api/rules/${targetDialect}/${filename}`, { credentials: 'same-origin' });
      ...
      <div className="fixed inset-0 bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-3">
            ...
            {currentLogLevel && (
              <span className={`px-2 py-1 rounded text-xs font-medium ${currentLogLevel === 'DEBUG'
        ...
        <div className="flex border-b">
          <button
            ...
            className={`px-6 py-3 font-medium ${activeTab === 'logs'
          ...
          <button
            onClick={() => setActiveTab('default-rules')}
            className={`px-6 py-3 font-medium ${activeTab === 'default-rules'
              ? 'border-b-2 border-blue-600 text-blue-600'
              : 'text-gray-600 hover:text-gray-900'
              }`}
          >
            <div className="flex items-center gap-2">
              ...
              <span>YAML Rules (Default)</span>
            </div>
          </button>
          <button
            onClick={() => setActiveTab('tsql-rules')}
            className={`px-6 py-3 font-medium ${activeTab === 'tsql-rules'
              ? 'border-b-2 border-blue-600 text-blue-600'
              : 'text-gray-600 hover:text-gray-900'
              }`}
          >
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {/* Rules Tab */}
          {(activeTab === 'tsql-rules' || activeTab === 'default-rules') && (
                ...
                <div className="divide-y">
                  {filteredRules.map((rule) => (
                    <button
                      ...
                      className={`w-full text-left p-3 hover:bg-blue-50 transition-colors ${selectedRule === rule.filename ? 'bg-blue-100' : ''
                        }`}
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className={`px-2 py-1 rounded text-xs ${rule.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'
                          }`}>
