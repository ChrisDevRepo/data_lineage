...
import { API_BASE_URL, SQL_DIALECT } from './config';
...
function DataLineageVisualizer() {
    ...
    const fetchMetadata = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Fetch database enabled status from metadata (v0.10.1 fix - no delay)
          if (metadata.db_enabled !== undefined) {
            setDbEnabled(metadata.db_enabled);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    const loadLatestData = async () => {
      ...
      // In demo/debug mode, try to load from localStorage cache first for instant loading
      if (runMode === 'demo' || runMode === 'debug') {
        try {
          const cached = localStorage.getItem('demo_data_cache');
          if (cached) {
          ...
          // If cache fails, continue to generate sample data
        }
      }
      ...
      try {
        ...
        const sampleData = generateSampleData();
        setAllData(sampleData);
        // Cache sample data for future loads
        if (runMode === 'demo' || runMode === 'debug') {
          try {
            localStorage.setItem('demo_data_cache', JSON.stringify(sampleData));
          } catch (e) {
  ...
  const handleResetView = useCallback(() => {
    // Reset view controls AND filter preferences to defaults
    // Reset: highlights, focus, search, excludeTerms, trace mode, hideIsolated, filterExtended
    // Preserve: schema and type selections (user's current filter scope)
    setHighlightedNodes(new Set());
    setFocusedNodeId(null);
    setSearchTerm('');
    setExcludeTerm(''); // Clear exclude input field
    setActiveExcludeTerms([]); // Clear active exclude filters
    setHideIsolated(true); // Reset to default: hide isolated nodes
    setFilterExtended(false); // Reset to default: show all schemas
    setFocusSchemas(new Set()); // Clear focus schemas
  ...
  // Don't block UI rendering - show shell immediately and load data in background
