...
file_handler = TimedRotatingFileHandler(
    ...
    backupCount=settings.log_retention_days,  # Keep N days of logs
    encoding='utf-8'
)
...
# Use centralized settings (can be overridden via ALLOWED_ORIGINS env var)
from engine.config import settings

app.add_middleware(
    CORSMiddleware,
    allow_origins=settings.allowed_origins_list,  # Centralized in settings
    allow_credentials=True,
    allow_methods=["GET", "POST", "DELETE"],  # Only methods we use
    allow_headers=["Content-Type", "X-Requested-With", "X-MS-CLIENT-PRINCIPAL"],
)
...
async def get_latest_data(user: Optional[Dict[str, Any]] = Depends(verify_azure_auth)) -> JSONResponse:
    ...
    try:
        ...
        with open(LATEST_DATA_FILE, 'rb') as f:  # Read as bytes for orjson
            raw_data = f.read()
        read_time = (time.time() - read_start) * 1000
        logger.debug(f"[API] File read took {read_time:.2f}ms")

        # Time the JSON parse (using orjson for 2-3x faster parsing)
...
async def upload_parquet(
    ...
    # Log cleanup now handled automatically by TimedRotatingFileHandler (v0.10.0)
    # No manual cleanup needed - logs rotate at midnight and keep N days automatically
    ...
    try:
        ...
        for upload_file in files:
            ...
            # Use async file I/O for non-blocking uploads (v0.10.0)
            import aiofiles
...
async def test_database_connection(
    ...
    try:
        ...
        # Extract server and database info from connection string
        server_info = ""
        db_info = ""

        if settings.db.connection_string:
            conn_str = settings.db.connection_string

            # Extract database name
            # TSQL format: DRIVER={...};SERVER=...;DATABASE=...;UID=...;PWD=...
            db_match = re.search(r'DATABASE=([^;]+)', conn_str, re.IGNORECASE)
            server_match = re.search(r'SERVER=([^;]+)', conn_str, re.IGNORECASE)

            if server_match:
                server_info = server_match.group(1).replace('tcp:', '')
            if db_match:
...
async def refresh_from_database(
    ...
    # Generate job ID (MUST use same format as upload-parquet for consistency)
    job_id = str(uuid.uuid4())
    job_dir = get_job_dir(job_id)
    job_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        # CRITICAL: Initialize status file BEFORE database fetch so frontend can poll
        status_data = {
            "status": "processing",
            "progress": 5.0,
            "current_step": "Connecting to database",
            "message": f"Connecting to database ({settings.sql_dialect})...",
            "elapsed_seconds": 0.0
        }
        with open(job_dir / "status.json", 'w') as f:
            json.dump(status_data, f, indent=2)
        ...
        status_data.update({
            "progress": 10.0,
            "current_step": "Fetching metadata",
            "message": "Fetching database objects (tables, views, procedures, functions)..."
        })
        with open(job_dir / "status.json", 'w') as f:
            json.dump(status_data, f, indent=2)
        ...
        if not result.success:
            # Database fetch failed - update status and release lock
            status_data.update({
                "status": "failed",
                "progress": 0,
                "current_step": "Error",
        ...
        # Update status: Ready for processing
        status_data.update({
            "status": "pending",
            "progress": 15.0,
        ...
        if incremental:
            stats = f"{result.new_procedures} new, {result.updated_procedures} updated, {result.unchanged_procedures} unchanged"
        else:
            stats = f"{result.total_procedures} total"

        # Count object types from imported data
        import pandas as pd
        objects_df = pd.read_parquet(job_dir / "objects.parquet")
        type_counts = objects_df['object_type'].value_counts().to_dict()

        # Map object type codes to readable names
        type_names = {
            'P': 'Stored Procedures',
            'V': 'Views',
            'U': 'Tables',
        ...
        type_summary = ", ".join([
            f"{type_counts.get(code, 0)} {type_names.get(code, code)}"
            for code in ['U', 'V', 'P', 'FN', 'TF', 'IF']
            if type_counts.get(code, 0) > 0
        ])

        return UploadResponse(
            job_id=job_id,
            message=f"Database refresh started ({mode_text} mode). Imported: {type_summary or f'{result.total_procedures} objects'} ({stats}).",
            files_received=["objects.parquet", "dependencies.parquet", "definitions.parquet"]
        )
...
async def list_rules(
    ...
    Args:
        dialect: SQL dialect (currently tsql only)
