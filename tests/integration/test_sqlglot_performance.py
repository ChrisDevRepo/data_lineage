...
class TestPhantomObjectDetection:
    ...
    def test_phantom_objects_table_exists(
        self, db_connection: duckdb.DuckDBPyConnection, table_exists_checker
    ):
        """Test that phantom_objects table exists if phantoms detected."""
        has_phantom_table = table_exists_checker('phantom_objects')

        # Phantom table may or may not exist depending on configuration
        # This is informational only
        if not has_phantom_table:
            pytest.skip("phantom_objects table not found (no external dependencies)")

    def test_phantom_objects_count(self, db_connection: duckdb.DuckDBPyConnection):
        """Test phantom object count is reasonable."""
        tables = db_connection.execute("SHOW TABLES").fetchall()
        table_names = [row[0] for row in tables]

        if 'phantom_objects' not in table_names:
            pytest.skip("phantom_objects table not found")

        result = db_connection.execute("""
            SELECT COUNT(*) FROM phantom_objects
        """).fetchone()

        phantom_count = result[0] if result else 0

        # Phantom count should be reasonable (not thousands)
        # High phantom count may indicate configuration issue
        if phantom_count > 0:
            assert phantom_count < 1000, (
                f"Unexpectedly high phantom count: {phantom_count} "
                f"(may indicate config issue)"
            )

    def test_phantom_objects_have_schema_and_name(
        self, db_connection: duckdb.DuckDBPyConnection
    ):
        """Test that phantom objects have valid schema and names."""
        tables = db_connection.execute("SHOW TABLES").fetchall()
        table_names = [row[0] for row in tables]
        ...
        for obj_id, schema, name, obj_type in results:
            assert obj_id < 0, f"Phantom {schema}.{name} should have negative ID"
            assert schema, f"Phantom object {obj_id} should have schema name"
            assert name, f"Phantom object {obj_id} should have object name"
            assert obj_type, f"Phantom object {obj_id} should have object type"
